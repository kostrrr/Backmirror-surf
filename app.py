...
def heat_color(p):
    if p == 100: return "#b00000"      # Alarm-Rot
...
# Beispielhafte Sessions (Simulation)
RAW = [
    {"day":"Mittwoch","start":"15:15","end":"16:00","type":"Basic","max":10,"used":6},
    {"day":"Freitag","start":"18:45","end":"19:30","type":"Basic Intense","max":5,"used":5},  # 100 %
...
def prepare_sessions():
    ...
    for s in RAW:
        ...
        # Employee: immer anzeigen, nie zählen
        if s["type"] == "Employee":
            out.append({
                "day": s["day"],
                "top": top,
        ...
        # ✅ NEUE REGEL:
        # 100 % immer erfassen, sonst nur im Zeitfenster
        if percent < 100 and not in_time_window(s["start"]):
            continue
...
<div class="calendar">
<div></div>{% for d in days %}<div class="header">{{d}}</div>{% endfor %}
<div>11:00–22:00</div>{% for d in days %}<div class="day" id="c{{d}}"></div>{% endfor %}
</div>

{% for s in sessions %}
<div class="session" data-day="{{s.day}}"
